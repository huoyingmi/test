...
db.on('open',()=>{
    ...
    // 4-3.更新数据操作
    /*
    UserModel.updateOne({age:{$gt:90}},{$set:{age:100}},(err,result)=>{
        if(err){
            console.log('updateOne user err::',err);
        }else{
            console.log(result);
        }
    })
    */
    /*
    UserModel.updateOne({age:{$gt:90}},{age:150},(err,result)=>{
        if(err){
            console.log('updateOne user err::',err);
        }else{
            console.log(result);
    ...
    */
    UserModel.updateMany({age:{$gt:90}},{age:150},(err,result)=>{
        if(err){
            console.log('updateOne user err::',err);
        }else{
            console.log(result);
